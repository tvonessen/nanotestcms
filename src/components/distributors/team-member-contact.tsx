...
export default function TeamMemberContact({ contact }: { contact: TeamMember }) {
  return (
    <div className="flex flex-col p-2 md:p-4 bg-primary/10 rounded-lg grow">
      <h3 className="ms-1 font-semibold text-lg leading-none">{contact.name}</h3>
      <h4 className="ms-1 text-sm text-primary-600 dark:text-primary-400">
        {contact.position} at NANOTEST
      </h4>
      <div className="flex flex-row gap-2">
        {contact.email && (
          <a className="mt-3" href={`mailto:${contact.email}`}>
            <Button isIconOnly={false} size="sm" radius="md" color="primary" variant="solid">

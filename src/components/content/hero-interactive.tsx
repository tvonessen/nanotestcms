...
import type { HeroInteractiveProps } from '@/components/content/hero';
import RichTextWrapper from '@/components/content/richtext-wrapper';
import { resolveAssetURL } from '@/utils/public-url';

const HeroInteractive = ({ images, showCaption = false, className }: HeroInteractiveProps) => {
      ...
      >
        ...
        >
          {images.map((image, index) => (
            <div className="relative md:px-2 focus:outline-hidden" key={image.id}>
              <Image
                key={image.filename}

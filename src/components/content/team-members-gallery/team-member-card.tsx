...
export default function TeamMemberCard({ member }: { member: TeamMember }) {
  ...
  return (
    <div
      key={member.id}
      className="group relative flex items-center team-member-card w-64 hover:scale-150 focus-visible:scale-150 transition-transform aspect-square"
      tabIndex={-1}
    >
      <svg
        width="100%"
      ...
      >
        ...
        />
        <g className="saturate-0 group-hover:saturate-100 transition-[filter]">
          <image
      ...
      <div className="absolute opacity-0 group-hover:opacity-100 inset-x-0 -bottom-2 w-fit mx-auto shadow-[0_-5px_10px_#0009] px-4 py-2 margin-auto bg-primary rounded-md transition-opacity">
        <h3 className="text-sm font-semibold text-foreground">{member.name}</h3>
        <p className="text-xs text-background">{member.position}</p>
      </div>
    </div>
  );
}

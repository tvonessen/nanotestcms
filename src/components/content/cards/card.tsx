...
interface CardProps {
  lang: string;
  content: NonNullable<RenderableCard>;
  className?: string;
}

export async function Card({ lang, content: data, className }: CardProps) {
  const content =
    data.type === 'solution'
      ? (data.solution as Solution)
      : {
          title: data.card.title,
          subtitle: data.card.subtitle,
          abstract: data.card.description,
          details: {
            images: data.card.image,
          },
          slug: typeof data.card.title?.replaceAll(' ', '-'),
          new: null,
          discontinued: null,
        };

  let raw = content.details.images;
        ...
        >
          <div className="max-h-[calc(100%-4.25rem)] w-full px-1 overflow-y-auto rounded-md scrollbar-hide">
            ...
            >
              ...
              {content.abstract && (
                <RichTextWrapper text={content.abstract} lang={lang} className="text-background" />
              )}
            </div>
          </div>
